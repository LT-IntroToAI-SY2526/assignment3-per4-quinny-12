...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    """
    start_year = matches[0]
    end_year = matches[1]

    result = []
    for movie in movie_db:
        if int(start_year) <= int(get_year(movie)) <= int(end_year):
            result.append(get_title(movie))
...
def title_before_year(matches: List[str]) -> List[str]:
    ...
    """
    year_limit = int(matches[0])
    
    result = []
    for movie in movie_db:
        if get_year(movie) < year_limit:
            result.append(get_title(movie))
...
def director_by_title(matches: List[str]) -> List[str]:
    ...
    """
    search_title = matches[0].lower()
    
    for movie in movie_db:
        if get_title(movie) == search_title:
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("who was in movies directed by %"), actors_by_director),
...
if __name__ == "__main__":
    ...
    assert sorted(title_after_year(["1990"])) == sorted(
        ["boyz n the hood", "dead again", "the crying game", "flirting", "malcolm x", "avengers: endgame"]
    ...
    assert sorted(actors_by_director(["john randal kleiser"])) == sorted(
        [
            "john travolta",
            "olivia newton-john",
            "didi conn",
